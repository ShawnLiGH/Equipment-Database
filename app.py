...
for pe, eq in rows:
    ...
    status = c2.selectbox("Status", ["TBD", "existing", "new", "replace", "remove"], index=["TBD", "existing", "new", "replace", "remove"].index(pe.status), key=f"st_{pe.id}")
    ...
    price = qc2.number_input("Unit Price", value=float(qt.price_value) if (qt and qt.price_value) else 0.0, step=100.0, key=f"p_{pe.id}")
